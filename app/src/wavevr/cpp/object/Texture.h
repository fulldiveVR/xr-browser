...
public:
  ...
  static uint8_t *
  cropBitmap(const uint8_t *origBitmap, const size_t origW, const size_t origH, const size_t x,
             const size_t y, const size_t w, const size_t h);

  inline void bindBitmap(int internalFormat = -1) {
    if (internalFormat != -1) {
      VRB_GL_CHECK(
          glTexImage2D(GL_TEXTURE_2D, 0, internalFormat, mWidth, mHeight, 0, mFormat, mType,
                       mBitmap)
      )
    } else {
      VRB_GL_CHECK(
  ...
  inline void bindTextureCubeMap() {
    VRB_GL_CHECK(glBindTexture(GL_TEXTURE_CUBE_MAP, mTexture))
  }

  inline void unbindTextureCubeMap() {
    VRB_GL_CHECK(glBindTexture(GL_TEXTURE_CUBE_MAP, 0))
  }
